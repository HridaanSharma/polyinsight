...
export function EventGroupsTab() {
  ...
  // Phase 1: fetching the 500 markets from Polymarket
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-24 text-white/50">
        <Loader2 className="w-8 h-8 animate-spin text-blue-400" />
        <p className="text-sm text-white/60">Loading live markets from Polymarket…</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-24 text-white/50">
        ...
        <div className="text-center">
          <p className="text-sm font-medium text-red-300">Failed to load markets</p>
  ...
  // Phase 2: markets loaded, Claude is analyzing them
  if (aiLoading) {
    return (
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between px-1">
        <div className="flex items-center gap-3 flex-wrap">
          <span className="text-sm text-white/60">
            {chains.length} causal chain{chains.length !== 1 ? "s" : ""} found
          </span>
          {totalMoved > 0 && (
      ...
      {chains.length === 0 ? (
        <div className="text-center py-16 text-white/40">
          <p className="text-sm">No causal chains found — try refreshing to re-run the analysis.</p>
          <Button variant="ghost" size="sm" onClick={() => refetch()} className="mt-3 text-white/40 hover:text-white/70">
